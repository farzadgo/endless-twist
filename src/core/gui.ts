...
// const subtitleElem = document.querySelector<HTMLParagraphElement>('#subtitle');
...
const showAbout = () => {
  ...
  duration.style.display = !aboutIsShown ? 'none' : 'block';
  ...
  if (!aboutIsShown) {    
    infoDiv!.innerHTML = `
      ...
      <section>
        ...
        <p><i> The project's source code is hosted publicly in <a href="https://github.com/farzadgo/endless-twist/" target="blank">this repository</a>. </i></p>
      </section>
    `
...
export const showOverlay = () => {
  ...
  if (!isMobile() && modelLoaded) overlay?.appendChild(guides);
  if (!isMobile() && modelLoaded) overlay?.appendChild(duration);

  // FOR THE SUBTITLE TIMESTAMPS
